...
$(THIS_ASSIGNMENT).pdf: $(TEX_DEPENDENCIES)
	...
	find "./" -name "*.tex" -exec sed -i -e '/.*START CODE HERE.*/,/.*END CODE HERE.*/{//!d;}' {} + ; \
	find "./" -name "*.tex" -exec sed -i -e '/.*BEGIN_HIDE.*/,/.*END_HIDE.*/{//!d;}' {} + ; \
	find "./" -name "*.tex" -exec sed -i -e '/SOLUTION ALERT/{N;d;}' {} + ; \
